// ── Parsing (module scope — shared by preview and render) ──
...
// ── Preview (shown in tile) ────────────────────────────────
export function preview(container, raw) {
  try {
    const { headers, rows } = parseAuto(raw);
    ...
    sample.forEach(row => {
      ...
      cols.forEach(h => { const td = tr.insertCell(); td.textContent = row[h] ?? ''; });
    });
    container.appendChild(t);

    const meta = document.createElement('div');
    meta.className = 'tile-meta';
    meta.textContent = `${rows.length.toLocaleString()} rows · ${headers.length} cols`;
...
export function render(container, raw) {
  ...
  function colTpl() { return `36px repeat(${headers.length}, minmax(120px, 1fr))`; }
  ...
  function process() {
    ...
    if (groupCol) {
      const groups = {};
      data.forEach(row => { const k = String(row[groupCol] ?? 'N/A'); (groups[k] = groups[k] || []).push(row); });
      displayed = [];
  ...
  function renderVirtual() {
    ...
    for (let i = start; i < end; i++) {
      ...
      if (item.type === 'g') {
        ...
        row.onclick = () => { collapsed.has(item.key) ? collapsed.delete(item.key) : collapsed.add(item.key); process(); };
  ...
  function editCell(cell, rowData, col) {
    ...
    input.value = orig;
    const commit = () => { rowData[col] = input.value; cell.textContent = input.value; input.remove(); editing = false; autoSave(); };
    input.onblur = commit;
    input.onkeydown = e => {
  ...
  function populateGroup() {
    ...
    headers.forEach(h => { const o = document.createElement('option'); o.value = h; o.textContent = h; groupEl.appendChild(o); });
  ...
  async function renderPanel() {
    ...
    dsList.innerHTML = '';
    if (!datasets.length) {
      const p = document.createElement('p'); p.className = 'dg-panel-empty'; p.textContent = 'No saved datasets.'; dsList.appendChild(p); return;
    }
    datasets.forEach(ds => {
      const item = document.createElement('div');
      item.className = 'dg-ds-item' + (ds.id === activeId ? ' active' : '');
      const info = document.createElement('div'); info.className = 'dg-ds-info';
      const name = document.createElement('div'); name.className = 'dg-ds-name'; name.textContent = ds.name;
      const meta = document.createElement('div'); meta.className = 'dg-ds-meta'; meta.textContent = `${ds.rowCount.toLocaleString()} rows · ${new Date(ds.timestamp).toLocaleDateString()}`;
  ...
  async function activate(id) {
    ...
    if (!ds) return;
    activeId = id; headers = ds.headers; rows = ds.rows;
  ...
  panelBtn.addEventListener('click', () => { panel.classList.toggle('open'); panelBtn.classList.toggle('active'); });
  dgEl.querySelector('#dg-panel-close').addEventListener('click', () => { panel.classList.remove('open'); panelBtn.classList.remove('active'); });

  dgEl.querySelector('#dg-export-json').addEventListener('click', () => {
    const data = displayed.filter(r => r.type === 'r').map(r => { const obj = {}; headers.forEach(h => { obj[h] = r.data[h] ?? ''; }); return obj; });
  ...
  async function init() {
    idb = await openDB();
    let parsed;
    try { parsed = parseAuto(raw); }
    catch (e) { container.innerHTML = `<div style="padding:var(--pad);font-family:var(--mono);font-size:.7rem">${e.message}</div>`; return; }
    if (!parsed.headers.length) {
      container.innerHTML = `<div style="padding:var(--pad);font-family:var(--mono);font-size:.7rem">No data found.</div>`; return;
    }
    ...
    activeId = id; headers = parsed.headers; rows = parsed.rows;
    populateGroup(); renderHead(); renderPanel(); process();
    toast(`${rows.length.toLocaleString()} rows loaded`);
  }
